...
### 6.3 Web Fetch Dynamic Filtering 对比

**测试场景：** "Please fetch the content at https://httpbin.org/html and count how many times 'hammer' appears"

该测试抓取 httpbin.org 提供的 Herman Melville《Moby-Dick》HTML 页面片段，要求 Claude 统计其中 "hammer" 一词的出现次数。这一场景覆盖了 Web Fetch 的完整链路：URL 抓取、HTML-to-text 转换、代码执行统计。

> **注：** 由于 Web Fetch 的官方 Anthropic API 对比测试环境尚未就绪，本节的两次运行均通过 Proxy（Bedrock）执行，作为**一致性验证**——确认同一实现在相同输入下的行为稳定性。Web Search 的对比（6.2 节）则是 Proxy 与官方 API 的直接对比。

#### Agentic Loop 行为

两次运行展现了**完全相同**的 3 轮迭代行为：

1. **Iteration 1**：Claude 调用 `web_fetch(url)` → httpx 抓取页面 → HTML-to-text 转换得到 3,602 字符的纯文本
2. **Iteration 2**：Claude 调用 `bash_code_execution` → 编写 Python 代码统计 "hammer" 出现次数 → 结果为 3 次
3. **Iteration 3**：Claude 基于代码执行结果生成最终文本回答

这一行为序列与 Anthropic 官方 API 的预期行为完全一致——Claude 先抓取内容，再通过代码进行精确统计，最后组织回答。

#### 基本指标

| 指标 | Run 1 | Run 2 | 差异 |
|------|-------|-------|------|
| input_tokens | 5,515 | 5,509 | -0.1% |
| output_tokens | 1,368 | 1,366 | -0.1% |
| content blocks | 7 | 7 | 相同 |
| web_fetch_requests | 1 | 1 | 相同 |

Token 差异不到 0.2%，几乎可以忽略不计。差异来源于 `retrieved_at` 时间戳的微秒级变化导致的 token 计数微调。

#### 关键发现

- **抓取内容一致**：两次运行中，httpx 从 httpbin.org 抓取的内容经过 HTML-to-text 清洗后**逐字节一致**（均为 3,602 字符）
- **代码执行一致**：Claude 生成了结构相同的 Python 代码，执行结果均为 3 次 "hammer" 出现
- **响应结构一致**：Content block 序列（text → server_tool_use → web_fetch_tool_result → text → server_tool_use → bash_code_execution_tool_result → text）在两次运行中完全相同
- **JSON 格式 100% 兼容**：`web_fetch_tool_result` 的嵌套结构（`web_fetch_result` → `document` → `source`）、`bash_code_execution_tool_result` 格式、`usage.server_tool_use.web_fetch_requests` 计数等均完全匹配

### 6.4 对比结论

综合 Web Search 和 Web Fetch 两组测试的结果，Proxy 实现与 Anthropic 官方 API 的对比总结如下：

| 评估维度 | 结果 |
|----------|------|
| Token 消耗 | 差异 <4%（自然波动范围内） |
| Agentic Loop 行为 | 搜索查询/迭代次数完全一致 |
| Dynamic Filtering | 代码执行结果完全一致 |
| 响应格式 | 100% 兼容（block types、ID 前缀、citation 格式、usage 字段） |
| 数据准确性 | 提取相同数据，得出相同结论 |
