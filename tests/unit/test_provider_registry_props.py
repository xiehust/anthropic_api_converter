...
_model_alphabet = string.ascii_lowercase + string.digits + "-_"

provider_name_st = st.text(alphabet=_name_alphabet, min_size=1, max_size=12)
model_id_st = st.text(alphabet=_model_alphabet, min_size=1, max_size=20)
model_set_st = st.frozensets(model_id_st, min_size=0, max_size=5)


@st.composite
def provider_st(draw):
    """Draw a StubProvider with a random name and set of supported models."""
...
class TestProviderRegistryModelLookup:
    ...
    **Validates: Requirements 1.2, 1.3, 1.4**
    ...
    def test_returns_exactly_supporting_providers(
        self, providers: list[StubProvider], query_model: str
    ):
        """
        **Validates: Requirements 1.2, 1.3, 1.4**

        get_providers_for_model returns exactly the set of providers whose
    ...
    def test_empty_list_when_no_provider_supports_model(
        ...
        **Validates: Requirements 1.4**
        ...
        When no registered provider supports the queried model, the result
        is an empty list.
        """
...
class TestProviderRegistryRoundTrip:
    ...
    For any provider, registering then unregistering should remove it from
    the registry.
    ...
    **Validates: Requirements 1.6**
    ...
    def test_register_then_unregister_removes_provider(self, provider: StubProvider):
        ...
        **Validates: Requirements 1.6**
        ...
        After register then unregister, get_provider returns None.
        """
        registry = ProviderRegistry()
    ...
    @settings(max_examples=150)
    def test_unregistered_provider_not_in_model_query(
        self, provider: StubProvider, query_model: str
    ):
        """
        **Validates: Requirements 1.6**

        After unregistering, the provider should not appear in
        get_providers_for_model results.
        """
        registry = ProviderRegistry()
        registry.register(provider)
        registry.unregister(provider.name)
