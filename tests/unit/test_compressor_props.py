...
_simple_messages = st.lists(
    st.one_of(
        st.builds(_make_user_msg, content=st.text(min_size=1, max_size=200)),
        st.builds(_make_assistant_msg, content=st.text(min_size=1, max_size=200)),
    ),
    min_size=1,
    max_size=20,
)
...
class TestCompressionOffPassthrough:
    ...
    **Validates: Requirements 12.4**
    ...
    def test_off_strategy_returns_unchanged(self, messages: list[dict]):
        ...
        # Messages should be the same object (not copied)
...
class TestHistoryFoldingStrategyAwareness:
    ...
    **Validates: Requirements 13.1, 13.2, 13.3, 13.4, 13.5**
    ...
    def test_aggressive_folds_old_long_messages(
        ...
        compressor = ContextCompressor(
            fold_after_turns=3, fold_min_length=200, fold_summary_length=150
        )
        ...
        # fold boundary: keep last fold_after_turns*2 = 6 messages
        ...
        for i, msg in enumerate(result):
            if i < fold_boundary and msg.get("role") == "assistant":
                ...
                assert len(text) <= 154  # fold_summary_length(150) + len("...")
...
class TestCompressionStatsAccuracy:
    ...
    **Validates: Requirements 14.2**
    ...
    @given(
        messages=st.lists(
            st.one_of(
                st.builds(_make_user_msg, content=st.text(min_size=1, max_size=500)),
                st.builds(
                    _make_assistant_msg, content=st.text(min_size=1, max_size=500)
                ),
            ),
            min_size=1,
            max_size=20,
