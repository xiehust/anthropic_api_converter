...
class TestBedrockProviderInvoke:
    ...
    @pytest.mark.asyncio
    async def test_invoke_delegates_to_bedrock_service(self):
        """invoke calls BedrockService.invoke_model with correct args."""
        mock_service = MagicMock()
...
class TestBedrockProviderInvokeStream:
    ...
    @pytest.mark.asyncio
    async def test_invoke_stream_delegates_to_bedrock_service(self):
...
class TestBedrockProviderGetCost:
    ...
    def test_get_cost_returns_zero_when_no_pricing_manager(self):
        ...
        cost = provider.get_cost("any-model", input_tokens=1000, output_tokens=500)
        assert cost == 0.0
    ...
    def test_get_cost_returns_zero_when_no_pricing_found(self):
        ...
        cost = provider.get_cost("unknown-model", input_tokens=1000, output_tokens=500)
        assert cost == 0.0
