...
_provider_name = st.sampled_from(["bedrock", "openai", "anthropic"])

_positive_price = st.floats(min_value=0.001, max_value=100.0, allow_nan=False, allow_infinity=False)
...
# ---------------------------------------------------------------------------
# Strategies for pricing item lists
# ---------------------------------------------------------------------------

@st.composite
def _pricing_items_with_availability(draw):
    """
    Generate a list of 2-8 pricing items with unique model IDs,
    distinct costs, and a random subset marked as available.
    ...
    prices = draw(
        st.lists(
            st.tuples(_positive_price, _positive_price),
            min_size=n,
            max_size=n,
        )
    )
    # Ensure all costs are distinct
...
def _all_unavailable_pricing(draw):
    ...
    models = draw(
        st.lists(_model_id, min_size=n, max_size=n, unique=True)
    )
    prices = draw(
        st.lists(
            st.tuples(_positive_price, _positive_price),
    ...
    **Validates: Requirements 8.1, 8.2, 8.3, 8.4**
    ...
    def test_cost_routing_selects_cheapest_available(self, data):
        ...
        **Validates: Requirements 8.1, 8.2, 8.3**
        ...
        # Compute expected: cheapest available model
        available_with_cost = []
        for p in items:
        ...
        assert decision.model == expected_model, (
            f"Expected cheapest available model '{expected_model}', "
    ...
    @given(data=_pricing_items_with_availability())
    @settings(max_examples=100)
    def test_cost_routing_skips_deprecated_models(self, data):
        """
        **Validates: Requirements 8.1, 8.3**

        Deprecated models are never selected by cost routing,
        even if they have the lowest cost.
        """
        items, available = data
        # Mark the cheapest item as deprecated
        all_costs = [
            (_std_cost(p["input_price"], p["output_price"]), i)
            for i, p in enumerate(items)
        ]
        all_costs.sort(key=lambda x: x[0])
        cheapest_idx = all_costs[0][1]
        items[cheapest_idx]["status"] = "deprecated"

        # Ensure at least one non-deprecated model is available
        non_deprecated_available = {
            p["model_id"] for p in items
            if p["model_id"] in available and p.get("status") != "deprecated"
        }
        assume(len(non_deprecated_available) > 0)

        engine = _make_engine(items, available)
        api_key_info = {"routing_strategy": "cost"}
        decision = engine.route("any-model", "hello", api_key_info)

        assert decision.model != items[cheapest_idx]["model_id"], (
            "Deprecated model should not be selected"
        )
        assert decision.model in non_deprecated_available
    ...
    **Validates: Requirements 11.3**
    ...
    def test_quality_routing_selects_most_expensive_available(self, data):
        ...
        **Validates: Requirements 11.3**
        ...
        # Compute expected: most expensive available model
        available_with_cost = []
        for p in items:
            if p["model_id"] in available and p.get("status") != "deprecated":
                cost = _std_cost(p["input_price"], p["output_price"])
        ...
        expected_model = available_with_cost[0][1]
        assert decision.model == expected_model, (
            f"Expected most expensive available model '{expected_model}', "
            f"got '{decision.model}'"
        )
...
class TestBudgetAwareDegradation:
    ...
    **Validates: Requirements 10.1, 10.2, 10.3**
    ...
    @given(
        budget=st.floats(min_value=1.0, max_value=10000.0, allow_nan=False, allow_infinity=False),
        ratio=st.floats(min_value=0.81, max_value=5.0, allow_nan=False, allow_infinity=False),
    ...
    def test_degrades_to_weak_model_when_budget_threshold_met(
        ...
        **Validates: Requirements 10.1, 10.2**
        ...
        assert decision.model == weak_model, (
            f"Strategy '{strategy}': expected weak_model '{weak_model}', "
            f"got '{decision.model}'"
        )
    ...
    @given(
        budget=st.floats(min_value=1.0, max_value=10000.0, allow_nan=False, allow_infinity=False),
        ratio=st.floats(min_value=0.0, max_value=0.79, allow_nan=False, allow_infinity=False),
        request_model=_model_id,
    )
    @settings(max_examples=100)
    def test_no_degradation_below_threshold(self, budget, ratio, request_model):
        ...
        **Validates: Requirements 10.1, 10.2**
        ...
        # Use a single cheap pricing item so cost routing succeeds
        ...
        assert decision.reason != "budget_degradation", (
            f"Should not degrade at ratio {ratio:.2f}"
        )

    @given(
        budget=st.floats(min_value=1.0, max_value=10000.0, allow_nan=False, allow_infinity=False),
        ratio=st.floats(min_value=0.8, max_value=5.0, allow_nan=False, allow_infinity=False),
        request_model=_model_id,
    )
    @settings(max_examples=100)
    def test_strategy_off_skips_budget_check(self, budget, ratio, request_model):
        ...
        **Validates: Requirements 10.3**
        ...
        assert decision.model == request_model, (
            f"Strategy 'off' should return original model '{request_model}', "
            f"got '{decision.model}'"
        )
    ...
    @given(
        request_model=_model_id,
        used=st.floats(min_value=0.0, max_value=10000.0, allow_nan=False, allow_infinity=False),
    )
    @settings(max_examples=100)
    def test_zero_budget_never_degrades(self, request_model, used):
        ...
        **Validates: Requirements 10.1**
        ...
        for budget_val in [0, -1, 0.0]:
            ...
            decision = engine.route(request_model, "hello", api_key_info)
            assert decision.reason != "budget_degradation", (
                f"Should not degrade when budget={budget_val}"
            )
